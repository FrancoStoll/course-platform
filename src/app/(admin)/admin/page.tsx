...
import { formatNumber, formatPrice } from "@/lib/firnatters";
...
async function AdminPage() {
  ...
  return (
    <div className="container my-6">
      <div className="grid gridcols1 sm:grid-cols-3 lg:grid-cols-5 md:grid-cols-4 gap-4">
...
async function getPurchaseDetails() {
  ...
  const [refundedData] = data.filter((row) => row.isRefund);
  const [salesData] = data.filter((row) => !row.isRefund);

  const netSales = (salesData?.totalSales ?? 0) / 100;

  const totalRefunds = (refundedData?.totalSales ?? 0) / 100;
  const netPurchases = salesData?.totalPurchases ?? 0;
  const refundedPurchases = refundedData?.totalPurchases ?? 0;
